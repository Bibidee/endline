...
async function send(address:`0x${string}`,functionName:string,args:unknown[]){const c=writeClient(address);await c.connect("studionet");const hash=await c.writeContract({address:contractAddress!,functionName,args:args as any,value:BigInt(0)});const receipt=await readClient().waitForTransactionReceipt({hash,status:TransactionStatus.FINALIZED});if(receipt.statusName!=="FINALIZED"||receipt.resultName!=="MAJORITY_AGREE"||receipt.txExecutionResultName!=="FINISHED_WITH_RETURN")throw new Error("Finalised transaction did not execute successfully");return hash}
