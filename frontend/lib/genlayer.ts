...
export function findMatchingRegistration(candidates:Dependency[],creator:string,canonicalKey:string,sources:string[]){const matches=candidates.filter(d=>d.creator.toLowerCase()===creator.toLowerCase()&&d.canonical_key===canonicalKey&&d.source_version===1&&d.assessment_count===0&&JSON.stringify(d.source_urls)===JSON.stringify(sources));if(matches.length!==1)throw new Error(matches.length>1?"RECONCILIATION ERROR: registration is ambiguous":"RECONCILIATION ERROR: registration readback not found");return matches[0]}
export function findAssessmentForVersion(assessments:Assessment[],beforeSequence:number,sourceVersion:number){return assessments.find(a=>a.sequence>beforeSequence&&a.source_version===sourceVersion)||null}
async function send(address:`0x${string}`,functionName:string,args:unknown[]){const c=writeClient(address);const hash=await c.writeContract({address:contractAddress!,functionName,args:args as any,value:BigInt(0)});const receipt=await readClient().waitForTransactionReceipt({hash,status:TransactionStatus.FINALIZED,interval:3000,retries:60});if(!transactionSucceeded(receipt))throw new Error("Finalised transaction did not execute successfully");return hash}
